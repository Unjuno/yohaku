...
for (const limit of [0,-1,21,32,"NaN"]) {
...
assert.equal(compat.selection_scope,"recommended_subset");assert.equal(compat.total_available,32);
...
for(const query of ["","静かな","怖くないSF","探偵","ホラー","潜入","音楽","園芸","町づくり","ダンス","zzzz"]){
...
console.log(JSON.stringify({play_count:32,compat_count:compat.count,worker_rest_mcp:"PASS",review_projection:"PASS",browser_voice_llm:"NOT_TESTED"},null,2));
