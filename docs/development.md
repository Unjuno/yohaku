...
This document preserves the implementation and local-development notes that originally lived in the repository README.
...
- `.openai/hosting.json` declares optional Sites D1 and R2 bindings and is intentionally ignored by Git
...
- `npm test`: source contract checks
- `npm run test:types`: type-check the changed pure modules
- `npm run check:sync`: check that checkout-local paths are not tracked
- `npm run verify`: build and run Worker REST/MCP integration checks
